...
- (void)viewDidLoad {
    ...
    NSString * path = [[NSBundle mainBundle] pathForResource:@"kyrgistan-1" ofType:@"html"];
...
- (void) segmentedControlTapped:(UISegmentedControl *)paramSender{
    ...
    switch (paramSender.selectedSegmentIndex) {
        ...
        {
            NSLog(@"Right pressed");
            NSString *path = [[NSBundle mainBundle] pathForResource:@"Cambodia" ofType:@"html"];
            NSURL *url = [NSURL fileURLWithPath:path];
            NSMutableURLRequest *urlRequest = [NSMutableURLRequest requestWithURL:url];
            [self.webContent loadRequest:urlRequest];
        }
